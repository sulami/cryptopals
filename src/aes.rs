...
fn inv_sub_bytes(word: u32) -> u32 {
    let mut bs: [u8; 4] = word.to_be_bytes();
    for i in 0..4 {
        let first_four_bits = bs[i] >> 4;
        let last_four_bits = bs[i] & 0x0f;
        let idx = first_four_bits * 16 + last_four_bits;
        bs[i] = INV_S_BOX[idx as usize];
    }
    u32::from_be_bytes(bs)
}
...
fn mix_columns(state: &mut State) {
    for ncol in 0..4 {
        ...
        state[0*4+ncol] = MUL_2[column[0] as usize]
            ^ MUL_3[column[1] as usize]
            ^ column[2]
            ^ column[3];
        state[1*4+ncol] = column[0]
            ^ MUL_2[column[1] as usize]
            ^ MUL_3[column[2] as usize]
            ^ column[3];
        state[2*4+ncol] = column[0]
            ^ column[1]
            ^ MUL_2[column[2] as usize]
            ^ MUL_3[column[3] as usize];
        state[3*4+ncol] = MUL_3[column[0] as usize]
            ^ column[1]
            ^ column[2]
            ^ MUL_2[column[3] as usize];
    }
}
...
fn inv_mix_columns(state: &mut State) {
    for ncol in 0..4 {
        ...
        state[0*4+ncol] =
            MUL_14[column[0] as usize] ^
            MUL_11[column[1] as usize] ^
            MUL_13[column[2] as usize] ^
            MUL_9[column[3] as usize];
        state[1*4+ncol] =
            MUL_9[column[0] as usize] ^
            MUL_14[column[1] as usize] ^
            MUL_11[column[2] as usize] ^
            MUL_13[column[3] as usize];
        state[2*4+ncol] =
            MUL_13[column[0] as usize] ^
            MUL_9[column[1] as usize] ^
            MUL_14[column[2] as usize] ^
            MUL_11[column[3] as usize];
        state[3*4+ncol] =
            MUL_11[column[0] as usize] ^
            MUL_13[column[1] as usize] ^
            MUL_9[column[2] as usize] ^
            MUL_14[column[3] as usize];
    }
}
...
fn inv_sub_column_bytes(state: &mut State) {
    for column in 0..4 {
        let new_column = inv_sub_bytes(
            u32::from_be_bytes([
                state[0*4+column],
                state[1*4+column],
                state[2*4+column],
...
pub fn decrypt(input: State, key: Key) -> State {
    let mut state = input;
    let keys = expand_key(&key);
    ...
    for round in 1..10 {
        ...
        inv_sub_column_bytes(&mut state);
        add_round_key(&mut state, keys[10-round]);
